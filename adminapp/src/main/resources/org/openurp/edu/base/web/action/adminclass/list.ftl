...
[@b.grid items=adminclasses var="adminclass"]
  ...
  [@b.row]
    ...
    [@b.col width="7%" property="code" title="代码"]${adminclass.code}[/@]
    [@b.col width="23%" property="name" title="名称"][@b.a href="!info?id=${adminclass.id}"]${adminclass.name}[/@][/@]
    [@b.col width="5%" property="grade" title="年级"]${adminclass.grade!}[/@]
    [@b.col width="6%" property="education" title="培养层次"]${(adminclass.education.name)!}[/@]
    [@b.col width="15%" property="department" title="院系"]${(adminclass.department.name)!}[/@]
    [@b.col width="18%" property="major" title="专业(方向)"]${(adminclass.major.name)!} ${(adminclass.direction.name)!}[/@]
    [@b.col width="9%" property="stdType" title="学生类别"]${(adminclass.stdType.name)!}[/@]
    [@b.col width="5%" property="stdCount" title="人数"]${adminclass.stdCount!}[/@]
    [@b.col width="9%" property="instructor" title="辅导员"]${(adminclass.instructor.user.name)!}[/@]
  [/@]
[/@]
...
<script>
  ...
  function getExportData(value){
    ...
    if(value=='adminclass'){
      ...
      bg.form.addInput(document.searchForm,"titles","班级代码,班级名称,年级,教学项目,院系,培养层次,专业,方向,学生类别,计划人数,学籍有效人数,创建时间,修改时间");
