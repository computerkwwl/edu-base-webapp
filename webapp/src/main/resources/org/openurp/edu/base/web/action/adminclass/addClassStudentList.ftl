...
[@b.head /]
	[@b.form name="searchStdForm" id="searchStdForm" action='!addClassStudentList' method="post"]
		...
		[@b.grid sortable="false" items=studentStates! var="ss" style='width:98%;' filterable='true']
			[@b.gridfilter property="std.code"]<input type="text" style="width:90%;" name="studentState.std.code" value="${(studentState.std.code)!}"/>[/@]
			[@b.gridfilter property="std.person.name"]<input type="text" style="width:90%;" name="studentState.std.person.name.formatedName" value="${(studentState.std.person.name.formatedName)!}"/>[/@]
			[@b.gridfilter property="std.person.gender.name"]${studentState.std.person.gender.name}[/@]
			[@b.gridfilter property="grade"]<input type="text" style="width:90%;" name="student.grade" value="${studentState.grade!}"/>[/@]
			[@b.gridfilter property="department.name"]<input type="text" style="width:90%;" name="studentState.department.name" value="${(studentState.department.name)!}"/>[/@]
			[@b.gridfilter property="major.name"]<input type="text" style="width:90%;" name="studentState.major.name" value="${(studentState.major.name)!}"/>[/@]
			[@b.gridfilter property="adminclass.name"]<input type="text" style="width:90%;" name="studentState.adminclass.name" value="${(studentState.adminclass.name)!}"/>[/@]
			[@b.row]
				[@b.boxcol/]
				[@b.col width="12%" property="std.person.code" title="学号"/]
				[@b.col width="10%" property="std.name" title="姓名"/]
